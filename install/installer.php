...
if(!is_file("$mod_db")) {
	...
	$sql_conversations_table = generate_sql_query($mod_root.'install/tpl-conversations.php');
	$sql_prefs_table = generate_sql_query($mod_root.'install/tpl-preferences.php');
	$sql_projects_table = generate_sql_query($mod_root.'install/tpl-projects.php');
	$sql_tasks_table = generate_sql_query($mod_root.'install/tpl-tasks.php');
	$sql_timers_table = generate_sql_query($mod_root.'install/tpl-timers.php');
	$sql_activities_table = generate_sql_query($mod_root.'install/tpl-activities.php');
	$sql_clients_table = generate_sql_query($mod_root.'install/tpl-clients.php');
	$sql_catalog_table = generate_sql_query($mod_root.'install/tpl-stock.php');
	$sql_categories_table = generate_sql_query($mod_root.'install/tpl-categories.php');
	
	$dbh = new PDO("sqlite:$mod_db");
	
	$dbh->query($sql_conversations_table);
	$dbh->query($sql_projects_table);
	$dbh->query($sql_tasks_table);
	$dbh->query($sql_timers_table);
	$dbh->query($sql_activities_table);
	$dbh->query($sql_clients_table);
	$dbh->query($sql_prefs_table);
	$dbh->query($sql_catalog_table);
...
if($prefs_version < $mod['version']) {
	...
	$all_tables = glob($mod_root.'install/tpl-*.php');
